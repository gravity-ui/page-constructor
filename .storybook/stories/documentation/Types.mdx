...
- `desktop: string` — Desktop image URL, used as a fallback for every breakpoint
- `mobile?: string` — Mobile image URL
- `tablet?: string` — Tablet image URL

At least one of `tablet` / `mobile` is required: with `desktop` alone this renders exactly like [`ImageObjectProps`](#ImageObjectProps), so use `{src}` for a single image.
