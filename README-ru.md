...
- **События по умолчанию** — универсальные события Page Constructor для кнопок, ссылок, навигации и контролов. Для их отправки установите `autoEvents.enabled: true`.
- **Расширенные события** — зарегистрированные события, которые передаёт библиотека-композиция. Наличие `autoEvents.extendedEvents` включает их независимо от `enabled` и позволяет добавить префикс и счётчик.
- **Пользовательские события** задаются потребителями через `analyticsEvents`. Конфигурация автоматических событий их не изменяет.
