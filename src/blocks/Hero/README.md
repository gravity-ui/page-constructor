...
| `buttons`          | `HeroButton[] \| {light, dark}[]`                        | —           | CTA buttons. Each requires `text` + `url` and supports `theme`, `primary`, `target`, `analyticsEvents`, `extraProps`, `when`. Always rendered at size `xl`: a per-button `size` is accepted but ignored. |
