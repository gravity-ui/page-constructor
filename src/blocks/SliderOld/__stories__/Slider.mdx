...
`adaptive?: bool`: Enables adaptive height for slides.

`autoplay?: number`: Autoplay speed in milliseconds. Set to 0 or leave undefined to disable.

`disclaimer?: { text: string; size?: TextSize }` — Disclaimer text displayed below the slider.
