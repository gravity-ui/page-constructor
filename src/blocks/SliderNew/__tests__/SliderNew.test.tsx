...
import SliderNew from '../Slider';

const EXAMPLE_URL = 'https://example.com';
const SLIDER_TITLE = 'Slider New title';
...
describe('SliderNew', () => {
    test.each(slidesToShowValues)('Has correct slider labels', async (slidesToShow) => {
        const {container} = render(
            <SliderNew title={{text: SLIDER_TITLE, url: EXAMPLE_URL}} slidesToShow={slidesToShow}>
                {Array(CARDS_COUNT)
                    .fill(null)
                    .map((_, index) => (
                        <BasicCard
        ...
        const barElement = container.querySelector('.pc-SliderNewBlock__bar');
        if (slidesToShow > 1) {
        ...
        // eslint-disable-next-line testing-library/no-container, testing-library/no-node-access
        const accessibleBarElement = container.querySelector('.pc-SliderNewBlock__accessible-bar');
